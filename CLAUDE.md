# Claude Code — kafka-message-scheduler-admin Instructions
...
- **`-tags musl` is mandatory** — all `go build`/`go test`/`go run` in `server/` require `-tags musl` (confluent-kafka-go static linking)
- **Three DB layers** — `coldDB` (bbolt+bleve+Kafka), `liveDB` (HTTP-only), `historyDB` (bbolt+bleve+history topic) — never mix routes
- **StoreResetType must clear downstream state** — `server/store/store.go:StoreResetType` triggers full wipe; ignoring it = stale schedules
- **Decoder errors are non-fatal** — `httpdecoder` logs warning + returns original schedule; do not make fatal
- **WatchableStore.Close() order** — stop Watch() consumers BEFORE calling Close() to avoid "send on closed channel" panic
- **REST API streams, not buffers** — `routes.go` writes JSON directly to `http.ResponseWriter` via channel range; never load full result into memory
- **Resolver partial results OK** — `ErrPartialResults` is acceptable; `ErrNoResults` = all schedulers unreachable = critical
- **Cross-layer field names differ** — Go JSON: `target-topic`, `target-key`; TypeScript: `targetTopic`, `targetId` — map carefully in `makeScheduleInfoModel`
...
- **[2024] Three-tier DB** — cold (bbolt+bleve), live (HTTP), history (bbolt+bleve) — not a single DB. Each view is independently queryable without cross-contamination.
- **[2024] `-tags musl` build tag** — static linking of librdkafka via confluent-kafka-go musl variant. Required on all builds/tests.
- **[2024] SPA + API in one binary** — React build embedded via `http.FileServer` unless `API_SERVER_ONLY=true`. No separate frontend server in production.
- **[2024] DNS-based service discovery** — scheduler instances discovered via DNS lookup of `SCHEDULERS_ADDR` hosts + HTTP `/info` endpoint. No service registry.
...
- **NEVER omit `-tags musl`** from Go build/test commands — confluent-kafka-go will fail without it
- **NEVER buffer full schedule result sets in memory** — use channel streaming as in `routes.go`
- **NEVER commit `.db/` directory** — bbolt + bleve data files; gitignored
- **NEVER modify IDE configuration files** (`.idea/`, `.vscode/settings.json`) unless the user explicitly asks

## Don't

- Don't add a package manager config (yarn/pnpm/bun) — server uses Go modules, client uses npm
- Don't add DB migrations — this project has no SQL DB; bbolt+bleve require no schema migrations
- Don't query `liveDB` in tests that don't need live scheduler instances — use `coldDB` or `historyDB` mocks
- Don't bypass the `store.Store` interface with direct bbolt calls — always go through the interface
- Don't add auth middleware — this admin is internal/network-protected; no auth layer is intentional
