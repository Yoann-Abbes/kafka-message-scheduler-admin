# GitHub Copilot — kafka-message-scheduler-admin Instructions
...
| schedule lifecycle, store events, WatchableStore | `claude/lifecycle.md` |
| database, bbolt, bleve, coldDB, liveDB, historyDB | `claude/database.md` |
| adapters, REST store, HTTP decoder, resolver | `claude/adapters.md` |
| webhooks, event delivery, Watch() channel, streaming | `claude/webhooks.md` |
...
- **`-tags musl` mandatory** — all `go build`/`go test`/`go run` in `server/` require `-tags musl`
- **Three DB layers** — cold (bbolt+bleve+Kafka), live (HTTP), history (bbolt+bleve) — never mix routes
- **StoreResetType must clear downstream state** — handle all three EventTypes: Upsert, Deleted, StoreReset
- **Decoder errors non-fatal** — `httpdecoder` logs warning, returns original schedule
- **Cross-layer field name mismatch** — Go JSON: `target-topic`/`target-key`; TS: `targetTopic`/`targetId`
- **Stream, don't buffer** — REST API streams `chan schedule.Schedule` directly to ResponseWriter
