...
| Argument | Action |
|----------|--------|
| `all` | `cd server && go test -v -tags musl -count=1 ./...` |
| `<pkg-path>` | `cd server && go test -v -tags musl -count=1 ./<pkg-path>/...` |
| `ci` | `cd server && go test -v -tags musl -count=1 ./...` |
| `coverage` | `cd server && go test -cover -tags musl ./...` |
| `integration` | `cd server && RUN_INTEGRATION_TESTS=yes go test -v -tags musl -failfast -count=1 ./...` |
