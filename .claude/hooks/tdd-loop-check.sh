...
if [ -f 'package.json' ]; then
  ...
  TEST_CMD='go test -v -tags musl -count=1 ./...'
  ...
  if [ -f 'nx.json' ]; then
    ...
    if [ "$LINT_CMD" = 'golangci-lint run' ] && grep -q '"lint"' package.json 2>/dev/null; then
      LINT_CMD='npm run lint'
    fi
    if [ "$LINT_CMD" != 'golangci-lint run' ] && [ -n "$LINT_CMD" ]; then
